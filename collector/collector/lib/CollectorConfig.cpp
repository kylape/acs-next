...
StringEnvVar nats_url("NATS_URL");
StringEnvVar cluster_id("CLUSTER_ID");
...
void CollectorConfig::InitCollectorConfig(CollectorArgs* args) {
  ...
  // NATS broker configuration
  if (nats_url.hasValue()) {
    nats_url_ = nats_url.value();
    CLOG(INFO) << "NATS URL configured: " << *nats_url_;
  }
  if (cluster_id.hasValue()) {
    cluster_id_ = cluster_id.value();
    CLOG(INFO) << "Cluster ID configured: " << cluster_id_;
  }
