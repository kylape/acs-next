...
Service::Service(const CollectorConfig& config)
  ...
  // Select signal service client based on configuration:
  // 1. NATS broker (if NATS_URL is set and built with NATS support)
  // 2. gRPC to Sensor (if grpc_channel is set)
  // 3. Stdout (fallback)
#ifdef HAVE_NATS
  if (config.GetNatsUrl().has_value() && !config.GetNatsUrl()->empty()) {
    signal_client_ = std::make_unique<NatsSignalServiceClient>(
        *config.GetNatsUrl(), config.GetClusterId());
    CLOG(INFO) << "Using NATS signal client: " << *config.GetNatsUrl();
  } else
#endif
